...
wnut_iob = {'B-CORP': 0, 'I-CORP': 1, 'B-CW': 2, 'I-CW': 3, 'B-GRP': 4, 'I-GRP': 5, 'B-LOC': 6, 'I-LOC': 7, 'B-PER': 8,
            'I-PER': 9, 'B-PROD': 10, 'I-PROD': 11, 'O': 12}
...
def get_reader(file_path, gazetteer_path, max_instances=-1, max_length=128, target_vocab=None,
    ...
    reader.read_data(file_path, gazetteer=gazetteer_path)
...
def create_model(train_data, dev_data, tag_to_id, batch_size=64, dropout_rate=0.1, stage='fit', lr=1e-5,
    ...
    return NERBaseAnnotator(train_data=train_data, dev_data=dev_data, tag_to_id=tag_to_id, batch_size=batch_size,
                            stage=stage, encoder_model=encoder_model,
...
def load_model(model_file, tag_to_id=None, stage='test'):
    ...
    model = NERBaseAnnotator.load_from_checkpoint(model_file, hparams_file=hparams_file, stage=stage,
                                                  tag_to_id=tag_to_id)
    model.stage = stage
    return model
...
def get_trainer(gpus=4, is_test=False, out_dir=None, epochs=10):
    ...
    logger = CSVLogger(out_dir, name="lightning_logs")

    from pytorch_lightning.callbacks import ModelCheckpoint
    checkpoint_callback = ModelCheckpoint(monitor='val_macro@F1', mode='max', save_weights_only=True)

    if is_test:
        return pl.Trainer(gpus=1) if torch.cuda.is_available() else pl.Trainer(val_check_interval=100)

    if torch.cuda.is_available():
        trainer = pl.Trainer(logger=logger, gpus=gpus, deterministic=True, max_epochs=epochs,
                             callbacks=[checkpoint_callback],
                             default_root_dir=out_dir, distributed_backend='ddp')
        trainer.callbacks.append(get_lr_logger())
    else:
        trainer = pl.Trainer(logger=logger, max_epochs=epochs, default_root_dir=out_dir)
